...
mod tests {
    ...
    fn test_try_build_with_missing_owner() {
        ...
        let result = AccountBuilder::new().balance(100_000).try_build();
    ...
    fn test_create_pda() {
        ...
        let (pda, bump, account) =
            AccountBuilder::create_pda(&program_id, seeds, 100_000, test_data.clone()).unwrap();
    ...
    fn test_borsh_serialization() {
        ...
        let deserialized =
            borsh_serialization::deserialize_account_data::<TestBorshData>(&account).unwrap();
        assert_eq!(deserialized, test_data);
    }
    ...
    fn test_create_account_helper() {
        ...
        let (account_pubkey, account) = create_account(
            pubkey,
            AccountBuilder::new().balance(100_000).owner(program_id),
        )
        .unwrap();
    ...
    fn test_create_accounts() {
        ...
        let accounts = create_accounts(vec![
            ...
            (
                pubkey2,
                AccountBuilder::new().balance(200_000).owner(program_id),
            ),
        ])
        .unwrap();
    ...
    fn test_account_builder_default_owner() {
        // Create an account without specifying an owner
        let account = AccountBuilder::new().balance(100_000).build();
